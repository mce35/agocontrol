...
client.add_device(ex_temp, "temperaturesensor")
client.add_device(ex_umidity, "multilevelsensor")
				...
				if yahooweather_result <> "":
					...
					if temperatura <> old_temp:
						if (readTempUnits == 'f' or readTempUnits == 'F'):
							...
							client.emit_event(ex_temp, "event.environment.temperaturechanged", tempF, "F")
						else:
       			                 		client.emit_event(ex_temp, "event.environment.temperaturechanged", temperatura, "C")
					if umidita <> old_humidity:
		       				client.emit_event(ex_umidity, "event.environment.humiditychanged", umidita, "%")
