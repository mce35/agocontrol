...
class AgoEnigma2(agoclient.AgoApp):
    def setup_app(self):
        self.connection.add_handler(self.message_handler)

        hostsconfig = agoclient.config.get_config_option("enigma2", "hosts", "")
        try:
            hosts = map(str, hostsconfig.split(','))
        except:
            self.log.trace("no static hosts defined")
        else:
            for host in hosts:
                self.connection.add_device(host, "settopbox")

        self.log.info("discovering devices")
        s = myavahi.zmqconf(self.mycallback)
    ...
    def mycallback(self, name, host, port):
        if "dm500hd" in name or "dm600pvr" in name:
            ...
            try:
                ...
                self.log.trace("Found %s on %s\n" % (e2model.childNodes[0].data,str(host)))
                self.connection.add_device(str(host), "settopbox")
            except:
                self.log.error("ERROR: Can't determine enigma version")
